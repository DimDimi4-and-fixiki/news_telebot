...
class DataBaseHandler(object):
    ...
    def add_chat_id(self, **kwargs):
        """
        Adds chat_is to the user

        """
        telegram_id = str(kwargs.get("telegram_id", None))
        chat_id = str(kwargs.get("chat_id", None))
        query = "UPDATE User SET Chat_id = " + chat_id + \
                " WHERE Telegram_id = " + telegram_id
        print("NEW FEATURE", query)
    ...
    def get_users_by_time(self, **kwargs):
        """
        Gets telegram IDs of users with particular time
        :param kwargs:
            news_time: str  time in format "hh:mm"
        """
        news_time = str(kwargs.get("time", None))
        query = "SELECT Telegram_id FROM User WHERE News_time = \"" + news_time + "\""
        res = self.make_select_query(query=query)
        ids = []
        for id in res:
            ids.append(id[0])
    ...
    def get_user_chat_id(self, **kwargs):
        """
        Gets user's chat_id
        """
        telegram_id = str(kwargs.get("telegram_id", None))
        query = "SELECT Chat_id FROM User WHERE Telegram_id = " + str(telegram_id)
        res = self.make_select_query(query=query)
        return res[0][0]
    ...
    def update_user_first_time_enter(self, **kwargs):
        """
        Sets user's First_time_enter to 'False'
        :param kwargs:
        :return:
        """
        telegram_id = kwargs.get("telegram_id")
        query = "UPDATE USER SET First_time_enter = \"False\" WHERE Telegram_id = " \
                + str(telegram_id)
        print("FIRST TIME UPDATE: ", query)
        self.make_update_query(query=query)
    ...
    def get_user_num_of_articles(self, **kwargs):
        """
        gets number of articles user selected
        :param kwargs:
                telegram_id: telegram id of the user
        :return: str    user's num_of_articles
        """
        telegram_id = kwargs.get("telegram_id", None)
        query = "SELECT Num_of_articles FROM User WHERE Telegram_id = " + str(telegram_id)
        res = self.make_select_query(query=query)
        return str(res[0][0])
    ...
    def get_user_country(self, **kwargs):
        ...
        :return:         str    the user's country
        """
        telegram_id = kwargs.get("telegram_id", None)  # user's telegram ID
        query = "SELECT Country FROM User WHERE Telegram_id = " + str(telegram_id)
        res = self.make_select_query(query=query)
        return str(res[0][0])
